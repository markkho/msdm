...
from pyrlap_old.core.util import sample_prob_dict, SANSRTuple, SANSTuple
from pyrlap_old.core.transition_function import TransitionFunction
...
class MDP(object):
    ...
    def get_reachable_transition_reward_functions(self,
        ...
        if init_states is None:
            init_states = list(self.get_init_state_dist().keys())
    ...
    def as_matrices(self,
        ...
        if only_reachable:
            ss = list(self.get_reachable_states())
        else:
            ss = self.get_states()
        ...
        sa_rf = np.einsum("sar,sar->sa", tf, rf)
        ...
        if as_torch:
            ...
            sa_rf = torch.Tensor(sa_rf).type(tensor_dtype).to(torch_device)


        return {
            'tf': tf, 'rf': rf, "sa_rf": sa_rf,
            's0': s0, 'ss': ss, 'aa': aa,
